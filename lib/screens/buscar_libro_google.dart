...
class _BuscarLibroGoogleScreenState extends State<BuscarLibroGoogleScreen> {
  ...
  Future<void> _buscarLibro(String texto) async {
    ...
    if (_fuente == 'Google Books') {
      final url = Uri.parse(
          'https://www.googleapis.com/books/v1/volumes?q=${Uri.encodeComponent(texto)}');
      ...
      // OpenLibrary
      final url = Uri.parse(
          'https://openlibrary.org/search.json?q=${Uri.encodeComponent(texto)}');
      final respuesta = await http.get(url);
      if (respuesta.statusCode == 200) {
        final data = json.decode(respuesta.body);
        setState(() {
  ...
  Future<void> _agregarLibro(dynamic libroData) async {
    ...
    if (_fuente == 'Google Books') {
      ...
      nuevoLibro = Libro(
        tipoArchivo: '', // No hay archivo al crear desde API
        id: '',
        uid: uid,
        titulo: libro['title'] ?? 'Sin título',
        autor: (libro['authors'] != null
            ? (libro['authors'] as List).join(', ')
            : 'Desconocido'),
      ...
      nuevoLibro = Libro(
        ...
        autor: (libroData['author_name'] != null
            ? (libroData['author_name'] as List).join(', ')
            : 'Desconocido'),
