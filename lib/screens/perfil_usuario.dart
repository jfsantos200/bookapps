...
class _PerfilUsuarioState extends State<PerfilUsuario> {
  ...
  void initState() {
    super.initState();
    // Asegura que la UI ya está montada antes de cargar datos
  ...
  Future<void> _cargarDatos() async {
    ...
    if (!mounted) return; // Evita usar setState si el widget fue desmontado
    setState(() {
  ...
  Future<void> _cambiarFotoPerfil() async {
    ...
    if (pickedFile != null) {
      ...
      try {
        ...
        if (!mounted) return; // Verifica que el widget sigue activo
      ...
      if (mounted) setState(() => loadingFoto = false);
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            // --- CÍRCULO AVATAR CON BOTÓN PARA CAMBIAR FOTO ---
            Center(
              child: Stack(
                children: [
                  CircleAvatar(
            ...
            Text('$nombre $apellido',
                style: Theme.of(context).textTheme.titleLarge?.copyWith(
                    fontWeight: FontWeight.bold, color: AdminLteColors.dark)),
            const SizedBox(height: 8),
            Text('Edad: $edad',
                style: Theme.of(context)
                    .textTheme
                    .bodyMedium
                    ?.copyWith(color: AdminLteColors.gray)),
            const SizedBox(height: 8),
            Text(email,
                style: Theme.of(context)
                    .textTheme
